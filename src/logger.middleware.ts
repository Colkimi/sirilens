...
export class LoggerMiddleware implements NestMiddleware {
  use(req: Request, res: Response, next: NextFunction) {
    ...
    const authDetails = authorization
    ? `auth=present scheme=${authorization.split(' ')[0]} tokenLength=${authorization.split(' ')[1]?.length ?? 0}`
    : 'auth=missing';

    console.log(
    `[${new Date().toISOString()}] ${req.method} ${req.path} ${authDetails}`,
    );
